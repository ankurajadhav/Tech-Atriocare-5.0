  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl mx-auto relative px-4 md:px-16">
        ...
        <button 
          onClick={handlePrev}
          className="hidden sm:flex absolute sm:-left-4 top-1/2 -translate-y-1/2 sm:w-11 sm:h-11 rounded-full bg-white/95 backdrop-blur-sm border border-slate-200 items-center justify-center text-slate-700 shadow-sm sm:shadow-md hover:shadow-lg hover:gradient-colors active:scale-95 transition-all z-20 cursor-pointer"
          aria-label="Previous slide"
        >
          <ChevronLeft className="sm:w-5 sm:h-5" />
        </button>

        {/* Floating Side Right Control */}
        <button 
          onClick={handleNext}
          className="hidden sm:flex absolute sm:-right-4 top-1/2 -translate-y-1/2 sm:w-11 sm:h-11 rounded-full bg-white/95 backdrop-blur-sm border border-slate-200 items-center justify-center text-slate-700 shadow-sm sm:shadow-md hover:shadow-lg hover:gradient-colors active:scale-95 transition-all z-20 cursor-pointer"
          aria-label="Next slide"
        >
          <ChevronRight className="sm:w-5 sm:h-5" />
        </button>

        {/* Dynamic Slider Wrapper with smooth height transitions */}
        <div className="relative w-full min-h-fit md:min-h-[580px] flex items-center justify-center">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="w-full bg-white rounded-3xl border border-sky-100/80 shadow-[0_30px_70px_rgba(0,118,129,0.06)] overflow-hidden flex flex-col md:flex-row min-h-fit md:min-h-[520px]"
            >
              {/* Left Column: Visual Area */}
              <div className="w-full h-[240px] sm:h-[320px] md:h-auto md:flex-1 bg-slate-50 relative flex items-center justify-center overflow-hidden border-b md:border-b-0 md:border-r border-slate-100">
                ...
                {currentSlide.type === 'press-mock' ? (
                  <div className="w-full h-full flex flex-col bg-white">
                    ...
                    <div className="bg-slate-100 px-3 py-1.5 sm:px-4 sm:py-2 flex items-center justify-between border-b border-slate-200 flex-wrap text-[9px] sm:text-[10px] text-slate-600 font-sans tracking-wide">
                    ...
                    <div className="p-3 sm:p-5 md:p-6 flex-1 flex flex-col justify-between overflow-hidden relative">
                      {/* Side-by-side Layout inside the news article mock */}
                      <div className="grid grid-cols-1 sm:grid-cols-12 gap-3 sm:gap-4 items-start flex-1">
                        ...
                        <div className="hidden sm:block sm:col-span-7 space-y-1.5 sm:space-y-2 text-left">
                          <div className="text-[8px] sm:text-[10px] font-black tracking-widest text-slate-400 uppercase">BUSINESS NEWS INDEX</div>
                          <h4 className="text-xs sm:text-sm md:text-base font-black text-cyan-800 leading-snug tracking-tight">
                      ...
                      <div className="hidden sm:flex mt-2.5 pt-2 border-t border-slate-100 items-center justify-between gap-4 text-[8px] sm:text-[9px] text-slate-400 font-bold uppercase tracking-wider">
              ...
              <div className="flex-1 p-5 sm:p-8 md:p-12 flex flex-col justify-between text-left">
                ...
                <div className="space-y-3 sm:space-y-4">
                  ...
                  <h3 className="text-base sm:text-2xl md:text-3xl font-extrabold text-cyan-800 tracking-tight leading-snug">
                    {currentSlide.title}
                  </h3>
                  
                  <p className="text-emerald-800 font-bold text-xs sm:text-sm bg-emerald-50/70 border border-emerald-100 py-1 px-3 rounded-full w-fit">
                ...
                {/* Main Action Buttons */}
                <div className="mt-4 sm:mt-8 pt-4 sm:pt-6 border-t border-slate-100 flex flex-col sm:flex-row items-center gap-4">
                  {currentSlide.isExternal ? (
                    currentSlide.isNovicule ? (
                      <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                        <button
                          onClick={onOpenNoviculeModal}
                          className="px-5 py-3 sm:px-6 sm:py-3.5 rounded-xl sm:rounded-2xl bg-[#0097a7] hover:bg-[#0aa4b4] text-white font-bold text-xs uppercase tracking-widest transition-all cursor-pointer flex items-center justify-center gap-2"
                        >
                        ...
                        <a 
                          ...
                          className="px-5 py-3 sm:px-6 sm:py-3.5 rounded-xl sm:rounded-2xl border border-slate-200 text-slate-700 font-bold text-xs uppercase tracking-widest hover:bg-slate-50 transition-all flex items-center justify-center gap-2"
                        >
                      ...
                      <a 
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="w-full sm:w-auto px-6 py-3 sm:px-8 sm:py-4 rounded-xl sm:rounded-2xl bg-brand-blue hover:bg-brand-teal text-white font-black text-xs uppercase tracking-widest text-center transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2 cursor-pointer"
                      >
                    ...
                    {slides.map((_, idx) => (
                      <button
                        ...
                        className={`h-2 transition-all cursor-pointer ${
                          idx === currentIndex ? 'w-6 bg-[#0097a7] rounded-full' : 'w-2 bg-slate-200 hover:bg-slate-300 rounded-full'
                        }`}
